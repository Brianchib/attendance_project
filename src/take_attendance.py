...
def take_attendance(course_code):
    ...
    all_student_names = dict(
        cur.execute("SELECT name, index_number FROM student").fetchall()
    )
            ...
            for i, rect in enumerate(rects):
                ...
                for name, index_number in all_student_names.items():
                    if name == recog_data[i][0]:
                        present_student_res = cur.execute(
                            "SELECT name FROM attendance"
                        ).fetchall()
                        present_student_names = [
                            each for (each,) in present_student_res
                        ]
                        if name not in present_student_names:
                            ...
                            student_courses = cur.execute(
                                "SELECT course_code, course_date FROM attendance WHERE name = ?",
                                (name,),
                            ).fetchall()
